...
Rscript -e 'update.packages(repos="http://cran.rstudio.com/", ask=FALSE, checkBuilt=TRUE); to_install <- unname(installed.packages()[, "Package"]); install.packages(pkgs = to_install);'
sleep 10
# Install R packages
